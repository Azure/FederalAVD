...
| --------- | ----------- | :--: | :-----: |
| `identitySolution` | The service providing domain services for Azure Virtual Desktop.  This is needed to properly configure the session hosts and if applicable, the Azure Storage Account. | string | 'ActiveDirectoryDomainServices'<br/>'EntraDomainServices'<br/>'EntraId'<br/>'EntraKerberos-Hybrid'<br/>'EntraKerberos-CloudOnly' |
...
| --------- | ----------- | :--: | :-----: | ------- |
| `avdObjectId` | The Object ID for the Azure Virtual Desktop application in Entra Id with Application Id = '9cdead84-a844-4324-93f2-b2e6bb768d07'.  The Object ID can found by selecting Microsoft Applications using the Application type filter in the Enterprise Applications blade of Entra Id. If you use the custom UI and template spec, this value can be obtained by using the Microsoft Enterprise Application selector. Required when `deploymentType` = 'Complete' or 'HostpoolOnly' and you deploy a scaling plan or set the host pool properties to include PowerOnConnect. | string | object id | |
| `avdPrivateDnsZoneResourceId` | If using private endpoints with Azure Virtual Desktop, input the Resource ID for the Private DNS Zone used for feed download and connections to host pools. (privatelink.wvd.microsoft.com). Required when `avdPrivateLinkPrivateRoutes` is not set to 'None'. | string | resource id | '' |
| `azureBackupPrivateDnsZoneResourceId` | The resource Id of the Azure Backup private DNS zone. Required when `recoveryServices` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
| `azureBlobPrivateDnsZoneResourceId` | The resource Id of the Azure Blob private DNS zone. Required when `deployFSLogixStorage` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
| `azureFilesPrivateDnsZoneResourceId` | The resource Id of the Azure Files private DNS zone which is resolvable from the subnet where the session hosts are deployed. Required when `deployFSLogixStorage` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
| `azureFunctionAppPrivateDnsZoneResourceId` | The resource Id of the Azure Function App private DNS zone. Required when `deployScalingPlan` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
| `azureKeyVaultPrivateDnsZoneResourceId` | The resource Id of the Azure Key Vault private dns zone which is resolvable from the subnet that the session hosts will be placed upon. Required when `deploySecretsKeyVault` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
| `azureQueuePrivateDnsZoneResourceId` | The resource Id of the Azure Queue private DNS zone. Required when `deployFSLogixStorage` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
| `azureTablePrivateDnsZoneResourceId` | The resource Id of the Azure Table private DNS zone. Required when `deployFSLogixStorage` is true and `deployPrivateEndpoints` is true. | string | resource id | '' |
...
| `controlPlaneLocation` | The region to which the control plane resources should be deployed. If this is not set, the control plane will be deployed to the same region as the virtual machines and overall deployment. Required if `workspaceResourceId` is not provided. | string | region | '' |
...
| `existingHostsResourceGroupName` | The name of the resource group housing the compute objects (i.e., virtual machines, disks, nics, disk encryption sets, disk accesses, etc.) when the the `deploymentType` = 'SessionHostsOnly'. | string | | '' |
| `globalFeedPrivateDnsZoneResourceId` | If using private endpoints with Azure Virtual Desktop, input the Resource Id for the Private DNS Zone used for initial feed discovery. (privatelink-global.wvd.microsoft.com). Required when `avdPrivateLinkPrivateRoutes` is set to 'All'. | string | resource id | '' |
...
| `keyVaultPrivateEndpointSubnetResourceId` | The resource id of the subnet on which to create the secrets Key Vault private endpoint. Required when the `deploySecretsKeyVault` and `deployPrivateEndpoints` parameters are set to true. | string | resource id | '' |
...
| `workspaceFeedPrivateEndpointSubnetResourceId` | The resource ID of the subnet where the AVD Private Link endpoints will be created. Required when `avdPrivateLinkPrivateRoutes` is set to 'FeedAndHostPool' or 'All'. | string | resource id | '' |
...
| `agentBootLoaderDownloadUrl` | Custom URL or blob name for the AVD Agent Boot Loader MSI installer. When empty, defaults to publicly documented sources (go.microsoft.com links for public clouds, aka.ms perma-links for air-gapped clouds). Can be a full URL or blob name when using `artifactsContainerUri`. **See:** [AVD Agent installation reference](https://learn.microsoft.com/azure/virtual-desktop/agent-overview) | string | URL or blob name | '' |
| `agentDownloadUrl` | Custom URL or blob name for the AVD Agent MSI installer. The deployment always attempts to download the latest agent from the host pool API endpoint first. If that fails, it uses this custom URL (if provided) or defaults to publicly documented sources (go.microsoft.com links for public clouds, aka.ms permalinks for air-gapped clouds). Can be a full URL or blob name when using `artifactsContainerUri`. **See:** [AVD Agent installation reference](https://learn.microsoft.com/azure/virtual-desktop/agent-overview) | string | URL or blob name | '' |
...
| `deploymentType` | Determines whether the control plane, management, and compute resources are deployed or if the deployment will just add session hosts to an existing pool. | string | 'Complete'<br/>'HostpoolOnly'<br/>'SessionHostsOnly' | 'Complete' |
| `deploymentVmSize` | The size of the deployment virtual machine. | string | valid size | 'Standard_B2s' |
...
| `existingEncryptionKeyVaultResourceId` | The resource Id of the existing encryption keys key vault in the management resource group. Required when `deploymentType` = 'HostPoolOnly' and `keyManagementDisks` or `keyManagementStorageAccounts` contains 'CustomerManaged'. | string | resourceId | '' |
...
| `existingHostingPlanResourceId` | The resource Id of the existing hosting plan to use for the function app. | string | resourceId | '' |
...
| `existingRecoveryServicesVaultResourceId` | The resource Id of an existing Recovery Services Vault to use for backup. Required for `HostpoolOnly` and `SessionHostsOnly` deployment types when `recoveryServices` = true. For personal host pools this vault will receive VM backup items; for pooled host pools it will receive FSLogix Azure Files file share backup items. | string | resourceId | '' |
| `existingSharedActiveDirectoryConnection` | Determines if the shared active directory connection is used. | bool | true<br/>false | false |
...
| `fslogixAppUpdateUserAssignedIdentityResourceId` | The resource Id of the user assigned identity used to update the FSLogix application. | string | resourceId | '' |
...
| `fslogixOUPath` | The distinguished name of the target Organizational Unit in Domain Services where the FSLogix storage account computer objects will be located. | string | distinguished name | '' |
...
| `fslogixSizeInMBs` | The size of the FSLogix container in MBs. | int | | 30000 |
...
| `fslogixStorageKerberosEncryptionType` | The Active Directory computer object Kerberos encryption type for the Azure Storage Account or Azure NetApp files Account. | string | 'AES256'<br/>'RC4' | 'AES256' |
...
| `functionAppSubnetResourceId` | The resource Id of the subnet where the function app will be deployed. | string | resource id | '' |
| `hibernationEnabled` | Hibernation allows you to pause VMs that aren't being used and save on compute costs where the VMs don't need to run 24/7. **Note:** Hibernation does NOT support NVMe disks or Confidential VMs. Only specific VM sizes support hibernation (B-series v2, D/E-series v4/v5 with up to 64GB RAM, and NV-series v4/v5 with up to 112GB RAM). See [Azure VM hibernation documentation](https://learn.microsoft.com/en-us/azure/virtual-machines/hibernate-resume) for details. | bool | true<br/>false | false |
...
| `netAppVolumesSubnetResourceId` | The resource Id of the Virtual Network delegated for NetApp Volumes. Required when `fslogixStorageService` contains 'AzureNetAppFiles'. | string | resource id | '' |
| `permittedIPs` | Array of permitted IP addresses or CIDR blocks allowed through the firewall of all PaaS resources (storage accounts, Key Vaults). Use when managing the deployment from a trusted workstation outside the Azure network boundary. Leave empty to allow all public traffic (or rely solely on private endpoints). | array | IPv4 addresses or CIDR ranges | [] |
| `recoveryServices` | Enables Azure Backup via a Recovery Services Vault. For personal host pools, VM OS disks are enrolled in backup. For pooled host pools with `deployFSLogixStorage` = true and Azure Files, FSLogix file shares are enrolled in backup. A new vault is created for `Complete` deployments; an existing vault must be provided via `existingRecoveryServicesVaultResourceId` for `HostpoolOnly` and `SessionHostsOnly`. | bool | true<br/>false | false |
| `recoveryServicesVaultStorageRedundancy` | Controls the redundancy of backup recovery points stored inside the vault. For personal host pools (VM backup), `GeoRedundant` enables Cross-Region Restore, allowing VM recovery in a paired region if the primary is unavailable — recommended for mission-critical personal desktops. For pooled host pools (FSLogix/Azure Files backup), the actual share snapshots are stored in the storage account rather than the vault, so `LocallyRedundant` or `ZoneRedundant` is sufficient. Only applies when `deploymentType` = 'Complete' and `recoveryServices` = true. | string | 'LocallyRedundant'<br/>'ZoneRedundant'<br/>'GeoRedundant' (personal only) | 'LocallyRedundant' |
| `scalingPlanExclusionTag` | The tag used to exclude virtual machines from the scaling plan. | string | | '' |
| `scalingPlanForceLogoff` | Determines if the scaling plan forces logoff. | bool | true<br/>false | false |
| `scalingPlanMinsBeforeLogoff` | The number of minutes before logoff. | int | | 0 |
| `scalingPlanOffPeakSchedule` | The off peak schedule for the scaling plan. | object | | {} |
| `scalingPlanPeakSchedule` | The peak schedule for the scaling plan. | object | | {} |
| `scalingPlanRampDownSchedule` | The ramp down schedule for the scaling plan. | object | | {} |
| `scalingPlanRampUpSchedule` | The ramp up schedule for the scaling plan. | object | | {} |
| `secretsKeyVaultEnablePurgeProtection` | Determines if purge protection is enabled on the secrets key vault. | bool | true<br/>false | true |
| `secretsKeyVaultEnableSoftDelete` | Determines if soft delete is enabled on the secrets key vault. | bool | true<br/>false | true |
...
| `sessionHostCount` | The number of session hosts to deploy. | int | | 1 |
...
| `virtualMachinesTimeZone` | The time zone of the virtual machines. | string | valid time zone | 'Eastern Standard Time' |
| `vmNameIndexLength` | The length of the index for the virtual machine name. | int | | 3 |
| `vmOUPath` | The distinguished name of the target Organizational Unit in Domain Services where the session hosts computer accounts will be located. | string | distinguished name | '' |
| `workspaceFeedPublicNetworkAccess` | Determines if the AVD Workspace allows public network access when using AVD Private link. Applicable when `avdPrivateLinkPrivateRoutes` is set to 'FeedAndHostPool' or 'All'. 'Enabled' allows the global AVD feed to be accessed from both public and private networks, 'Disabled' allows this resource to only be accessed via private endpoints. | string | 'Disabled'<br/>'Enabled' | 'Enabled' |
...
| Parameter | Description | Type | Allowed | Default |
| --------- | ----------- | :--: | :-----: | ------- |
| `timeStamp` | **DO NOT INCLUDE IN PARAMETER FILES.** This value is automatically generated on each deployment using `utcNow()`. Used to create unique deployment suffixes and automatic image version numbers (e.g., `2026.0210.1435`). When saving parameters from Template Spec UI, always remove this parameter. Including it would reuse an old timestamp, defeating the purpose of automatic versioning and deployment uniqueness. | string | string | Auto-generated: `utcNow('yyyyMMddHHmmss')` |
...
| `imageDefinitionIsHibernateSupported` | Specifies whether the image definition supports creating VMs with support for hibernation. **Note:** Hibernation does NOT support NVMe disks or Confidential VMs. See [Azure VM hibernation documentation](https://learn.microsoft.com/en-us/azure/virtual-machines/hibernate-resume) for supported VM sizes and limitations. | bool | true<br/>false | false |
| `imageDefinitionIsHigherStoragePerformanceSupported` | Specifies whether the image definition supports capturing images of NVMe disks or Virtual Machines. **Note:** NVMe does NOT support hibernation or Confidential VMs. NVMe is available on v6 and newer generation VMs. See [Azure NVMe documentation](https://learn.microsoft.com/en-us/azure/virtual-machines/nvme-overview) for supported VM sizes. | bool | true<br/>false | false |
