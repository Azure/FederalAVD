...
| `agentDownloadUrl` | Custom URL or blob name for the AVD Agent MSI installer. Used when `useAgentDownloadEndpoint` is false, or as fallback if endpoint download fails when true. When empty, defaults to publicly documented sources (go.microsoft.com links for public clouds, aka.ms perma-links for air-gapped clouds). Can be a full URL or blob name when using `artifactsContainerUri`. **See:** [AVD Agent installation reference](https://learn.microsoft.com/azure/virtual-desktop/agent-overview) | string | URL or blob name | '' |
...
| `recoveryServices` | Determines if the recovery services vault is deployed. | bool | true<br/>false | false |
...
| `useAgentDownloadEndpoint` | When enabled (true), the deployment will attempt to download the latest AVD agent from the Azure Virtual Desktop service endpoint. If the endpoint download fails and `agentDownloadUrl` is provided, the system automatically falls back to that URL. When disabled (false), only `agentDownloadUrl` is used (or defaults if empty). **Recommended:** Leave enabled for public clouds to ensure latest agents. For air-gapped clouds, see [Air-Gapped Cloud Considerations](airGappedClouds.md#avd-agent-installation). | bool | true<br/>false | true |
