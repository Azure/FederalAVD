<#
.SYNOPSIS
    Restores final STIG policy after Packer provisioning and generalizes the Azure VM.

.DESCRIPTION
    Run this script as the final Packer provisioner after Apply-STIGsAVD.ps1 was invoked
    with -ExecutionProfile Packer. It removes the temporary Packer compatibility state,
    runs Sysprep, and waits until Windows reports that the image is generalized.

.PARAMETER IntendedDomainJoined
    Restores the domain-oriented firewall policy that disables local firewall-rule merge.
    Use this switch only when the captured image is guaranteed to join a domain.

.PARAMETER SysprepTimeoutMinutes
    Maximum time to wait for Sysprep to reach the generalized OOBE state.
#>
[CmdletBinding()]
param (
    [switch]$IntendedDomainJoined,

    [ValidateRange(5, 120)]
    [int]$SysprepTimeoutMinutes = 30
)

$ErrorActionPreference = 'Stop'
$requiredImageState = 'IMAGE_STATE_GENERALIZE_RESEAL_TO_OOBE'
$tempDirectory = Join-Path -Path $env:SystemRoot -ChildPath "Temp\Finalize-STIGsForPacker-$([guid]::NewGuid().ToString('N'))"
$logDirectory = Join-Path -Path $env:SystemRoot -ChildPath 'Logs\Configuration'
$logPath = Join-Path -Path $logDirectory -ChildPath 'Finalize-STIGsForPacker.log'
...
If (-not $windowsPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    throw 'Finalize-STIGsForPacker.ps1 must run from an elevated administrator session.'
}
...
Try {
    ...
    If (-not (Test-Path -LiteralPath $lgpoPath -PathType Leaf)) {
        throw "LGPO.exe was not found at '$lgpoPath'. Run Apply-STIGsAVD.ps1 before this finalizer."
    }

    Write-FinalizerLog -Message 'Restoring final registry-based policy before Sysprep.'
    $lgpoTextPath = Join-Path -Path $tempDirectory -ChildPath 'Packer-Final-Policy.txt'
    $null = New-Item -Path $lgpoTextPath -ItemType File -Force

    Add-LgpoRegistrySetting -FilePath $lgpoTextPath `
        -RegistryKeyPath 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' `
        -RegistryValue 'LocalAccountTokenFilterPolicy' `
        -RegistryData '0'
    Add-LgpoRegistrySetting -FilePath $lgpoTextPath `
        -RegistryKeyPath 'SOFTWARE\Policies\Microsoft\Windows\WinRM\Service' `
        -RegistryValue 'AllowBasic' `
        -RegistryData '0'
    ...
    # Also set the effective registry values now. LGPO persists them in local policy, while
    # these writes avoid relying on a later Group Policy refresh before image capture.
    Set-DwordRegistryValue `
    ...
    $seceditExport = Start-Process -FilePath 'secedit.exe' `
        -ArgumentList "/export /cfg `"$securityPolicyPath`" /areas USER_RIGHTS" `
        -Wait `
        -PassThru
    ...
    $existingPrincipals = @($securityPolicy[$denyNetworkLogonIndex].Split('=', 2)[1].Split(',') | ForEach-Object {
        $_.Trim()
    } | Where-Object { $_ })
    ...
    $seceditConfigure = Start-Process -FilePath 'secedit.exe' `
        -ArgumentList "/configure /db `"$securityDatabasePath`" /cfg `"$securityPolicyPath`" /areas USER_RIGHTS" `
        -Wait `
        -PassThru
    ...
    Write-FinalizerLog -Message 'Waiting for Azure Guest Agent services required by Sysprep.'
    ...
    Write-FinalizerLog -Message 'Starting Sysprep. Packer must not run another provisioner after this one.'
    ...
    $sysprep = Start-Process -FilePath $sysprepPath `
        -ArgumentList '/oobe /generalize /quiet /quit /mode:vm' `
        -Wait `
        -PassThru
    ...
    $sysprepDeadline = (Get-Date).AddMinutes($SysprepTimeoutMinutes)
    Do {
        $imageState = Get-ItemPropertyValue `
        ...
        If ((Get-Date) -ge $sysprepDeadline) {
            throw "Sysprep did not reach '$requiredImageState' within $SysprepTimeoutMinutes minutes."
        }
        Start-Sleep -Seconds 10
    } While ($true)

    Write-FinalizerLog -Message 'Windows is generalized. Returning control to Packer for power-off and image capture.'
}
Finally {
