...
| Capacity (50% / 85% / 95%) | Host Diagnostics / Utilization Report workbooks | `arg_max(TimeGenerated, *) by SessionHostName` pattern to get the latest row per host; `iff(AllowNewSessions and Status == 'Available', MaxSessions, 0)` for denominator to exclude drained hosts. Numerator uses `ActiveSessions + InactiveSessions` (aligned with Utilization Report workbook) — disconnected sessions (`InactiveSessions`) keep their slot on the host and count against `MaxSessions`, so omitting them caused capacity alerts to not fire when users disconnected without logging off. `InactiveSessions` is cast via `tolong(column_ifexists('InactiveSessions', 0))` for cloud compatibility. |
